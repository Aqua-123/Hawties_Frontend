...
class APIClient {
  ...
  async importData(id, file) {
    const formData = new FormData();
    formData.append('file', file);
    try {
      const response = await axiosClient.post(`/api/spreadsheets/${id}/ingest`, formData, {
        headers: {
  ...
  async addCollaborator(id, email, role) {
    try {
      const response = await axiosClient.post(`/api/spreadsheets/${id}/collaborators`, { email, role });
      return { success: true, data: response.data };
    } catch (error) {
  ...
  async removeCollaborator(id, email) {
    try {
      const response = await axiosClient.delete(`/api/spreadsheets/${id}/collaborators`, { data: { email } });
      return { success: true, data: response.data };
    } catch (error) {
